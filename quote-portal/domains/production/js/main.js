...
Object.assign(window, {
  ...
  onOperationsSearchInput, activateOperationRow, onOperationDefectRateInput, onOperationEfficiencyInput, cancelOperationEdit, saveOperationEdit, deleteOperationFromMaster,
  // time management
...
async function saveTimeManagement() {
  try {
    ...
    if (remoteOk) {
      ...
      // Refresh timeline UI with fresh data
      try {
        // Force cache refresh first
        const { getMasterData } = await import('./mesApi.js');
        await getMasterData(true);
        
        // Re-apply time settings to UI (this will reload blocks from fresh cache)
        await applyCompanyTimeSettingsToUI();
