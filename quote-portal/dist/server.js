...
// Version History API - Save previous version before admin changes
app.post('/api/quotes/:id/save-version', requireAuth, (req, res) => {
  try {
    const { id } = req.params
    const currentQuote = readOne(id)
    ...
    const versionSnapshot = {
      ...
      data: {
        ...
        proj: currentQuote.proj,
        phone: currentQuote.phone,
        email: currentQuote.email,
        customFields: currentQuote.customFields || {},
        price: currentQuote.price,
        status: currentQuote.status
      },
      reason: req.body.reason || 'Admin modification'
    }

    // Add to version history
    const patch = {
      versionHistory: [...(currentQuote.versionHistory || []), versionSnapshot]
    }

    updateOne(id, patch)
    res.json({ ok: true, version: versionSnapshot })
  } catch (error) {
...
app.get('/api/quotes/:id/versions', requireAuth, (req, res) => {
  try {
    const { id } = req.params
    const quote = readOne(id)
    ...
    res.json({ versions: quote.versionHistory || [] })
  } catch (error) {
...
app.post('/api/quotes/:id/restore-version', requireAuth, (req, res) => {
  try {
    const { id } = req.params
    const { versionIndex } = req.body
    const currentQuote = readOne(id)
    
    if (!currentQuote || !currentQuote.versionHistory) {
      return res.status(404).json({ error: 'Quote or version history not found' })
    }

    const targetVersion = currentQuote.versionHistory[versionIndex]
    if (!targetVersion) {
    ...
    const currentSnapshot = {
      ...
      reason: 'Auto-save before version restore'
    }

    // Restore version data
    const patch = {
      ...targetVersion.data,
      versionHistory: [...currentQuote.versionHistory, currentSnapshot],
      restoredFrom: {
        versionIndex,
    ...
    updateOne(id, patch)
    res.json({ ok: true, restored: targetVersion })
  } catch (error) {
